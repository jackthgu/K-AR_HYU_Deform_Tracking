...
void VisionProject::ReadSavedFrame()
{
	...
	colorMat = cv::imread(imageCntSS.str() + "c." + g_extension, cv::IMREAD_COLOR);
	cv::Mat depthMat8UC3 = cv::imread(imageCntSS.str() + "d." + g_extension, cv::IMREAD_COLOR);
...
void VisionProject::Render() {
	...
	cv::cvtColor(silhouetteMat, silhouetteBGR, cv::COLOR_GRAY2BGR);
...
void VisionProject::PostProcess() {
	...
	if ((refColor.empty() || reInit)) {
		...
		cv::Mat RoIMask = cv::Mat::zeros(prevGray.size(), CV_8UC1);
		cv::Rect roi(500, 300, 500, 300);
		RoIMask(roi).setTo(255);
		cv::goodFeaturesToTrack(refGray, wholeCorners, 1000, 0.01, 10);//, silhouetteMat);
		//cv::goodFeaturesToTrack(refGray, wholeCorners, 1000, 0.01, 10, RoIMask);//, silhouetteMat);
		...
		//if (opticalFlowDrawMask.empty())
		//{
		//	opticalFlowDrawMask = cv::Mat(refGray.rows, refGray.cols, CV_8UC3);
		//	opticalFlowDrawMask = cv::Mat::zeros(opticalFlowDrawMask.size(), CV_8UC3);
		//}
	...
	//} else if (!refZDRatio.empty()) {
		...
		//setRoI
		//initOpticalFlow
		opticalFlowDraw = colorMat.clone();
		if (setRoI)
		{
			if (!initOpticalFlow)
			{
				cv::cvtColor(colorMat, refGray, cv::COLOR_BGR2GRAY);
				prevGray = refGray.clone();

				std::vector<cv::Point2f> wholeCorners;
				cv::goodFeaturesToTrack(refGray, wholeCorners, 1000, 0.01, 10, objMask);
				cv::cornerSubPix(refGray, wholeCorners, subPixWinSize, cv::Size(-1, -1), termcrit);
			...
			{
				cv::Mat curGray;
				cv::cvtColor(colorMat, curGray, cv::COLOR_BGR2GRAY);
				std::vector<cv::Point2f> curCorners;
				//cv::goodFeaturesToTrack(curGray, curCorners, 1000, 0.01, 10);
				cv::goodFeaturesToTrack(curGray, curCorners, 1000, 0.01, 10, objMask);
				cv::cornerSubPix(curGray, curCorners, subPixWinSize, cv::Size(-1, -1), termcrit);

				for (uint i = 0; i < curCorners.size(); i++)
				{
					// Select good points
					//if (status[i] == 1) {
						//good_new.push_back(p1[i]);
						// draw the tracks
						//line(opticalFlowDrawMask, curCorners[i], prevCorners[i], RGB(255, 0, 0), 2);
					//cv::circle(opticalFlowDraw, curCorners[i], 2, RGB(255, 0, 0), 1);
					//cv::circle(colorMat, curCorners[i], 3, RGB(255, 0, 0), 2);
				...
				for (uint i = 0; i < curCorners.size(); i++)
				{
					...
					//if (status[i] == 1) 
					{
						//good_new.push_back(p1[i]);
						// draw the tracks
						//line(opticalFlowDraw, curCorners[i], prevCorners[i], RGB(0, 0, 255), 2);
						line(opticalFlowDraw, curCorners[i], prevCorners[i],cv::Scalar(0, 0, 255), 2);
						//cv::circle(opticalFlowDraw, curCorners[i], 2, RGB(255, 0, 0), 1);
					//cv::circle(colorMat, curCorners[i], 3, RGB(255, 0, 0), 2);
		...
		//std::cout << "prevCorners size : " << prevCorners.size() << std::endl;
		//for (uint i = 0; i < prevCorners.size(); i++)
		{
			// Select good points
			//if (status[i] == 1) {
				//good_new.push_back(p1[i]);
				// draw the tracks
				//line(opticalFlowDrawMask, curCorners[i], prevCorners[i], RGB(255, 0, 0), 2);
				//cv::circle(opticalFlowDrawMask, curCorners[i], 3, RGB(255, 0, 0), 2);
				//cv::circle(colorMat, curCorners[i], 3, RGB(255, 0, 0), 2);
...
int main(int argc, char *argv[])
{
	...
	//VisionProject::mInstance->InitializeGL();
	...
	cv::Mat opticalFlowVisual;
	//cv::Mat objMask;
	objMask = cv::Mat(FRAME_HEIGHT, FRAME_WIDTH, CV_8UC1);

	//cv::Mat hsvImg;
	//cv::cvtColor(colorMat, hsvImg, cv::COLOR_BGR2HSV);
	//int hMax, hMin, sMax, sMin;
	//int idx_x, idx_y;
	//idx_x = 640;
	//idx_y = 400;
	//hMax = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[0];
	//hMin = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[0];
	//sMax = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[1];
	//sMin = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[1];
	//int hVal, sVal, vVal;
	//int marginVal = 10;
	...
	//	{
	//		hVal = hsvImg.at<cv::Vec3b>(i, j)[0];
	//		sVal = hsvImg.at<cv::Vec3b>(i, j)[1];
	//		vVal = hsvImg.at<cv::Vec3b>(i, j)[2];

	//		if ((hVal > hMin - (marginVal) && hVal < hMax + (marginVal)))
	//		{
	//			objMask.at<uchar>(i, j) = 255;
	//		}
	//		else
	//		{
	//			objMask.at<uchar>(i, j) = 0;
	//		}
	//	}
	//}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//cv::Mat depthVisul;
		////depthMat.convertTo(depthVisul, CV_16SC1, 1.0 / 255.0);
		////depthMat.convertTo(depthVisul, CV_8UC1, 1.0 / 255.0);
		//cv::normalize(depthMat, depthVisul, 0, 255, cv::NORM_MINMAX);
		//cv::Mat mappedDepth;
		//depthMat.convertTo(mappedDepth, CV_8U, -255.0 / 5000.0, 255.0);

		cv::Mat hsvImg, yCvCr;
		cv::cvtColor(colorMat, hsvImg, cv::COLOR_BGR2HSV);
		cv::cvtColor(colorMat, yCvCr, cv::COLOR_RGB2YCrCb);
		int hMax, hMin, cbMax, cbMin;
		int idx_x, idx_y;
		int marginVal = 20;
		idx_x = 640;
		idx_y = 400;
		...
		if (g_frameCnt > 30)
		{
			if (!roiSet)
			{
				hMax = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[0] + marginVal;
				hMin = hsvImg.at<cv::Vec3b>(idx_y, idx_x)[0] - marginVal;
				cbMax = yCvCr.at<cv::Vec3b>(idx_y, idx_x)[2] + marginVal * 0.5;
				cbMin = yCvCr.at<cv::Vec3b>(idx_y, idx_x)[2] - marginVal * 0.5;
				roiSet = true;
			}
			else
			{
				...
				for (int i = 0; i < objMask.rows; i++)
				{
					...
					{
						hVal = hsvImg.at<cv::Vec3b>(i, j)[0];
						cbVal = yCvCr.at<cv::Vec3b>(i, j)[2];

						if ((hVal > hMin && hVal < hMax) && (cbVal > cbMin && cbVal < cbMax))
						{
							objMask.at<uchar>(i, j) = 1;
						}
						else
						{
							objMask.at<uchar>(i, j) = 0;
				...
				cv::Mat bin;
				threshold(objMask, bin, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
				//cv::imshow("binbinbin", bin);
				cv::Mat labels, stats, centroid;
				//int cnt = connectedComponentsWithStats(objMask, labels, stats, centroid);
				int cnt = connectedComponentsWithStats(bin, labels, stats, centroid);

				//std::cout << "Labeling : " << cnt << std::endl;

				int maxArea = 0;
				int max2Area = 0;
				int maxIdx = 0;
				for (int idx_label = 0; idx_label < cnt; idx_label++)
				{
					//std::cout << "max2Area : " << max2Area << " , " << stats.at<int>(idx_label, cv::CC_STAT_AREA) << "  --:: " << idx_label << std::endl;
					if (max2Area < stats.at<int>(idx_label, cv::CC_STAT_AREA))
					{
						if (maxArea < stats.at<int>(idx_label, cv::CC_STAT_AREA))
						{
							max2Area = maxArea;
							maxArea = stats.at<int>(idx_label, cv::CC_STAT_AREA);
						}
						else if (maxArea > stats.at<int>(idx_label, cv::CC_STAT_AREA))
						{
							max2Area = stats.at<int>(idx_label, cv::CC_STAT_AREA);
							maxIdx = idx_label;
							//std::cout << "maxIdx = idx_label;" << maxIdx << std::endl;
						}
					}
				}
				...
				for (int i = 0; i < objMask.rows; i++)
				{
					...
					for (int j = 0; j < objMask.cols; j++)
					{
						//if(labels.at<uchar>(i, j) == maxIdx+1)
						if(label[j] == maxIdx)
						{
							objMask.at<uchar>(i, j) = 255;
						}
				...
				cv::Mat stElem(3, 3, CV_8U, cv::Scalar(1));
				cv::erode(objMask, objMask, stElem, cv::Point(-1, -1), 5);

				setRoI = true;
			}
		}
		
		//cv::imwrite("test.png", colorMat);
		//objMask = cv::Mat::zeros(colorMat.size(), CV_8UC1);
		/*cv::cvtColor(colorMat, hsvImg, cv::COLOR_BGR2HSV);
		for (int i = 0; i < objMask.rows; i++)
		{
			for (int j = 0; j < objMask.cols; j++)
		...
		cv::imshow("objMask", objMask);

		//cv::applyColorMap(mappedDepth, mappedDepth, cv::COLORMAP_JET);

		//cv::imshow("DepthMat", mappedDepth);
		//cv::imshow("DepthMat", depthVisul);
		//add(colorMat, opticalFlowDrawMask, opticalFlowVisual);
		//cv::imshow("opticalFlowVisual", opticalFlowVisual);
		if (!opticalFlowDraw.empty())
		{
			cv::imshow("opticalFlowVisual", opticalFlowDraw);
		}
		cv::waitKey(1);
	}
